...
class FedNova(Optimizer):
    ...
    def __init__(self, params, ratio, gmf, mu = 0, lr=required, momentum=0, dampening=0,
                 weight_decay=0, nesterov=False, variance=0, slr=1.0,
        ...
        # No proximal for GeL
        if mu != 0:
            raise ValueError("Current version of the algorithm should not be used with proximal updates")
        if dampening != 0:
            raise ValueError("Current version of the algorithm uses no dampening")
        if nesterov:
            raise ValueError("Current version of the algorithm should not be used with Nesterov updates")
