    ...
    def average(self):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if 'global_second_moment_buffer' not in param_state: # This branch remains just like Adam
                    ...
                    delta_agg_squared = torch.clone(p.data**2).detach() # to be used twice
                    sign_of_diff = torch.sign(param_state['global_second_moment_buffer'] - delta_agg_squared) # sign(v^(t-1) - delta^(t)_agg^2)
                    param_state['global_second_moment_buffer'].sub_(1-self.beta2, delta_agg_squared * sign_of_diff) # v^(t) = v^(t-1) - (1-beta2) * sign * delta^(t)_agg^2
