...
save_dir=agg_testing_adv/cifar100
...
proxy_ratio=0.1
alphas=(0.5)
model=DeepSet_TM2 # we use trimmed mean at test time
train_model=DeepSet_M # we train on mean
trim_ratio=0.25
sizes=(17)
n_advs=(4)
black_box=false
        ...
        for seed in "${seeds[@]}"; do
            ...
            for attack_type in "${attacks[@]}"; do
                ...
                for n_adv in "${n_advs[@]}"; do
                    ...
                    name3=${dataset}_${alpha}_M${size}_${seed}_${proxy_ratio}_${train_model}_maxss${maxss}_minss${minss}_trim${trim_ratio}_attackpgd_bbfalse_colludetrue_nsubsets${n_subsets}_cw
                    modelpath=${root_dir}/results/agg_training_adv/cifar100/${name3}
